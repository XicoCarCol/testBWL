...
public class FilterTokenJWT extends OncePerRequestFilter {
    ...
    private final static Logger logger = LoggerFactory.getLogger(FilterTokenJWT.class);

    @Autowired
    private ProviderJWT providerJWT;

    @Autowired
    private UsuarioLogServiceImpl userDetailsService;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            if (token != null && providerJWT.validateToken(token)) {
                String nombreUsuario = providerJWT.getNombreUsuarioFromToken(token);
